...
#let project(fontsize:11pt, doc) ={
  let calculated_leading = 8pt
  let space_between_paragraphs= 1.1em
  /*
  set heading(
    bookmarked: true,
    numbering: (..nums) => {
      let nums_arr  = nums.pos().map(str)
      if (nums_arr.len() > 1) {
        none
      } else {numbering("I.", ..nums)}
    }
  )
  */
  ...
  show par: set block(spacing: space_between_paragraphs)
  set math.equation(
    numbering: "(1)"
  )
  show ref: it => {
